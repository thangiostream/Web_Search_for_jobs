...
function doInsert($jobid=0,$fileid=0) {
	...
	global $mydb; 
			...
			if ($age < 20){
			message("
			Tuổi không hợp lệ. 20 tuổi trở lên được phép.", "error");
			...
			message("
			Đơn đăng ký của bạn đã được gửi. Vui lòng đợi xác nhận của công ty nếu bạn đủ điều kiện cho công việc này.","success");
...
function doRegister(){
	...
	if (isset($_POST['btnRegister'])) { 
			...
			message("Bạn đã đăng ký thành công vào trang web. Bạn có thể đăng nhập ngay bây giờ!
			","success");
...
|| $imageFileType != "gif" ) {
		 if (move_uploaded_file($_FILES["picture"]["tmp_name"], $target_file)) {
			...
			message("
			Lỗi tải lên tệp","error");
			// redirect(web_root."index.php?q=apply&job=".$jobid."&view=personalinfo");
			// exit;
		}
	}else{
			message("
			Tệp không được hỗ trợ","error");
			// redirect(web_root."index.php?q=apply&job=".$jobid."&view=personalinfo");
			// exit;
		}
} 
